...
import {
  ...
  Loader2, Trash2, Flag, X, MapPin, Send, Bell, MessageSquare, ExternalLink
} from 'lucide-react'
  ...
  return (
    <Card className="overflow-hidden hover:shadow-lg transition-shadow border-border/60">
      ...
      <CardContent className="pb-3">
        ...
        {post.image_url && (
          <Link href={`/post/${post.id}`} className="block mt-3 rounded-lg overflow-hidden border border-border/40">
...
// ---------------- COMMENTS DIALOG (uses ThreadedComments) ----------------
function CommentsDialog({ post, open, onOpenChange, currentUser, supabase, onCountChange, onOpenProfile }) {
  if (!post) return null
  return (
      ...
      <DialogContent className="max-w-lg max-h-[85vh] flex flex-col p-0">
        <DialogHeader className="p-4 pb-2 border-b">
          <DialogTitle className="flex items-center justify-between">
            <span>Comments</span>
            <Link href={`/post/${post.id}`} className="text-xs font-normal text-orange-600 hover:underline flex items-center gap-1">
              Open post <ExternalLink className="h-3 w-3" />
            </Link>
          </DialogTitle>
        </DialogHeader>
        <div className="flex-1 overflow-y-auto px-4 py-3">
          <ThreadedComments postId={post.id} currentUser={currentUser} supabase={supabase} onOpenProfile={onOpenProfile} />
        </div>
      </DialogContent>
    </Dialog>
  )
...
function App() {
  ...
  useEffect(() => {
    ...
    // Unread messages: messages in my convos, from others, after my last_read_at
    const loadUnreadMsgs = async () => {
      ...
      if (!myParts || myParts.length === 0) { setUnreadMessages(0); return }
      let total = 0
        ...
        <div className="container mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <Link href="/messages">
            <Button variant="ghost" size="icon" className="relative">
              ...
              {unreadMessages > 0 && (
                <span className="absolute top-0.5 right-0.5 h-4 min-w-[16px] px-1 rounded-full bg-emerald-500 text-white text-[10px] font-bold flex items-center justify-center">
